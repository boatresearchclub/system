...
function renderBuy(rno){
  ...
  ranked.forEach(b => {
    ...
    // ── 展示補正係数 ──
    let tenjiCoef = 1.0;
    let tenjiRawP1 = null;    // テーブル参照直後の補間済み加減値(pt)。クランプなし。スコア計算に使用
    let tenjiRawBinP1 = null; // [2026-07-13 追加] 補間なしの本当の生テーブル値(整数)。表示専用
    if(tenjiScoreMap){
      tenjiCoef     = tenjiScoreMap[`__coef_${b.boat}`] ?? 1.0;
      tenjiRawP1    = tenjiScoreMap[`__rawP1_${b.boat}`] ?? null;
      tenjiRawBinP1 = tenjiScoreMap[`__rawBinP1_${b.boat}`] ?? null;
    }
    ...
    b._tenjiRawP1   = tenjiRawP1;   // テーブル生値（pt・補間済み）。7会場のみ非null
    b._tenjiRawBinP1 = tenjiRawBinP1; // 本当の生テーブル値（整数・補間なし）。7会場のみ非null
  ...
  // 「スリット補正」の表示スロットは、実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ/徳山/若松/丸亀/宮島）の
  // タイム由来1着率(p1)加算値を表示する用途に転用する（下のfinal_prob確定ループ内で設定）。

  // 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ/徳山/若松/丸亀/宮島）かどうか
  ...
  ranked.forEach(b => {
    ...
    if(isMeasuredTenjiVenue){
      ...
      // [2026-07-13] スリット補正表示用: 補間なしの本当の生テーブル値(整数)。
      //   スコア計算(上のclippedPt)には使わない。あくまで管理画面の参考表示。
      b.display_tenji_table_pt  = b._tenjiRawBinP1 ?? rawPt;
  ...
  const probRows = ranked2.map((bt,i)=>{
    ...
    // スリット補正列: [2026-07-13〜] 実測テーブル会場のみ、該当binの本当の生テーブル値
    //   （整数・補間なし）を表示。展示補正列（コース別クリップ後・補間済みの実効値）と
    //   見比べて、大きくズレていればクリップが効いている、という読み方ができる。
    let slitCorrCell;
    if(hasTenji && bt.display_tenji_table_pt != null){
      const rawPt = Math.round(bt.display_tenji_table_pt); // 整数（生テーブル値そのもの）
      if(rawPt === 0){
        slitCorrCell = `<span style="font-size:10px;color:var(--text3)">±0</span>`;
      } else {
        const color = rawPt >= 0 ? 'var(--green)' : 'var(--red)';
        const mark  = rawPt >= 0 ? '▲' : '▼';
        slitCorrCell = `<span style="font-size:10px;font-weight:600;color:${color}">${mark}${rawPt >= 0 ? '+' : '−'}${Math.abs(rawPt)}</span>`;
      }
    } else {
    ...
    // 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ/徳山/若松/丸亀/宮島）のみ、当日展示に基づく3連対率デルタ(__p3r_)を加算表示。
    ...
    <div style="padding:0.75rem 1.25rem 0.5rem;border-bottom:1px solid var(--border)">
      ...
      <table class="prob-table" style="width:100%;border-collapse:collapse">
        <thead><tr>
          ...
          <th class="admin-only" style="font-size:10px;color:var(--text3);font-weight:500;padding:3px 4px;text-align:center" title="[2026-07-13〜] 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ/徳山/若松/丸亀/宮島）のみ表示。該当diffが属する本当の生テーブル値(整数・補間なし)。展示補正列（コース別クリップ後・補間済みの実効値）と大きく異なる艇はクリップが効いている。それ以外の会場は—">スリット補正</th>
          ...
          <th class="admin-only" style="font-size:10px;color:var(--text3);font-weight:500;padding:3px 6px;text-align:center" title="出走表の3連対率（年間）に、実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ/徳山/若松/丸亀/宮島）のみ展示補正（3連対率デルタ）を加えた値。それ以外の会場は基準値のみ表示。6艇比較で最も低い艇には「切」バッジ表示">3連対率</th>
        </tr></thead>
