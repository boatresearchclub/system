...
function renderBuy(rno){
  ...
  // 「スリット補正」の表示スロットは、実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村）の
  // タイム由来1着率(p1)加算値を表示する用途に転用する（下のfinal_prob確定ループ内で設定）。

  // 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村）かどうか
    ...
    <div style="padding:0.75rem 1.25rem 0.5rem;border-bottom:1px solid var(--border)">
      ...
      <table class="prob-table" style="width:100%;border-collapse:collapse">
        <thead><tr>
          ...
          <th class="admin-only" style="font-size:10px;color:var(--text3);font-weight:500;padding:3px 4px;text-align:center" title="[2026-07-13〜] 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村）のみ表示。該当diffが属する本当の生テーブル値(整数・補間なし)。展示補正列（コース別クリップ後・補間済みの実効値）と大きく異なる艇はクリップが効いている。それ以外の会場は—">スリット補正</th>
