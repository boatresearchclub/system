...
function renderBuy(rno){
  ...
  // 「スリット補正」の表示スロットは、実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ）の
  // タイム由来1着率(p1)加算値を表示する用途に転用する（下のfinal_prob確定ループ内で設定）。

  // 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ）かどうか
  ...
  const probRows = ranked2.map((bt,i)=>{
    ...
    // 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ）のみ、当日展示に基づく3連対率デルタ(__p3r_)を加算表示。
    ...
    <div style="padding:0.75rem 1.25rem 0.5rem;border-bottom:1px solid var(--border)">
      ...
      <table class="prob-table" style="width:100%;border-collapse:collapse">
        <thead><tr>
          ...
          <th class="admin-only" style="font-size:10px;color:var(--text3);font-weight:500;padding:3px 4px;text-align:center" title="[2026-07-13〜] 実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ）のみ表示。該当diffが属する本当の生テーブル値(整数・補間なし)。展示補正列（コース別クリップ後・補間済みの実効値）と大きく異なる艇はクリップが効いている。それ以外の会場は—">スリット補正</th>
          ...
          <th class="admin-only" style="font-size:10px;color:var(--text3);font-weight:500;padding:3px 6px;text-align:center" title="出走表の3連対率（年間）に、実測テーブル会場（住之江/常滑/蒲郡/三国/鳴門/多摩川/平和島/戸田/芦屋/大村/児島/尼崎/びわこ）のみ展示補正（3連対率デルタ）を加えた値。それ以外の会場は基準値のみ表示。6艇比較で最も低い艇には「切」バッジ表示">3連対率</th>
        </tr></thead>
